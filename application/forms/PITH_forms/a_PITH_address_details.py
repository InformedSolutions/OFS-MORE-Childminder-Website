...
from application.models import (AdultInHomeAddress, ApplicantPersonalDetails)
...
class PITHAddressDetailsForm(ChildminderForms):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if AdultInHomeAddress.objects.filter(application_id=self.application_id_local, adult_in_home_address=1).count() > 0:
            self.fields['postcode'].initial = AdultInHomeAddress.objects.get(application_id=self.application_id_local,
                                                                    adult_in_home_address=1).postcode
...
class PITHAddressDetailsManualForm(ChildminderForms):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # If information was previously entered, display it on the form
        if AdultInHomeAddress.objects.filter(application_id=self.application_id_local, adult_in_home_address=1).count() > 0:
            pith_address_record = AdultInHomeAddress.objects.get(application_id=self.application_id_local, adult_in_home_address=1)
            self.fields['street_line1'].initial = pith_address_record.street_line1
            self.fields['street_line2'].initial = pith_address_record.street_line2
            self.fields['town'].initial = pith_address_record.town
            self.fields['county'].initial = pith_address_record.county
            self.fields['postcode'].initial = pith_address_record.postcode
            self.fields['country'].initial = pith_address_record.country
            self.pk = pith_address_record.adult_in_home_address_id
